...
public interface RecuRepository extends JpaRepository<Recu, Long> {
    ...
    @Query("SELECT r FROM Recu r WHERE r.payeur.id = :userId AND (LOWER(r.numero) LIKE LOWER(CONCAT('%', :q, '%')) OR LOWER(r.description) LIKE LOWER(CONCAT('%', :q, '%'))) ORDER BY r.dateEmission DESC")
